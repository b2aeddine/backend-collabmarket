...
-- ============================================================================
-- BACKEND MARKETPLACE INFLUENCEURS - VERSION PRODUCTION V20.0
-- ============================================================================
-- Base: V16.3 (ULTIME) + corrections V17.2 (revenus, sécurité) + analyse comparative
-- Objectifs V20 :
--   * Garder toutes les features métier & RPC de V16.3
--   * Conserver les RLS strictes et les vues publiques front
--   * Intégrer les améliorations critiques de V17.2 :
--       - Réversion revenus robuste dans safe_update_order_status()
--       - Revenus disponibles à finished + available_at = NOW()
--       - Regex delivery_url renforcée
--       - finalize_revenue_withdrawal en FIFO strict sans split
--       - RLS strict sur profiles + vue public_profiles
--   * Conserver le rate limiting avancé de V16.3
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.orders (
  ...
  delivery_url TEXT CHECK (
    delivery_url IS NULL
    OR delivery_url ~ '^https?://[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}.*'
  ),
...
CREATE INDEX idx_orders_acceptance_deadline
  ON public.orders(acceptance_deadline ASC)
  WHERE status = 'payment_authorized' AND acceptance_deadline IS NOT NULL;

DROP INDEX IF EXISTS idx_orders_merchant_deadline;
CREATE INDEX idx_orders_merchant_deadline
  ON public.orders(merchant_confirm_deadline ASC)
  WHERE status IN ('submitted','review_pending') AND merchant_confirm_deadline IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_revenues_available
  ON public.revenues(influencer_id, created_at) WHERE status = 'available';
CREATE INDEX IF NOT EXISTS idx_revenues_influencer_created
  ON public.revenues(influencer_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_reviews_influencer
  ON public.reviews(influencer_id, created_at DESC) WHERE is_visible = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_bank_accounts_default
  ON public.bank_accounts(user_id, is_default) WHERE is_default = TRUE;
...
DROP INDEX IF EXISTS uq_contestation_order_active;
CREATE UNIQUE INDEX uq_contestation_order_active
...
-- ============================================================================
-- 4. LOGIQUE MÉTIER (TRIGGERS HELPERS)
-- ============================================================================
...
BEGIN
  ...
  p_limit := CASE p_endpoint
    ...
    WHEN 'profile_view'  THEN 120
    WHEN 'profile_share' THEN 30
    ELSE p_limit
  END;
...
BEGIN
  ...
  IF p_role = 'admin' THEN
    IF p_new IN ('completed','finished')
       ...
       AND p_stripe NOT IN ('captured','succeeded') THEN
      RETURN FALSE;
    END IF;
    RETURN TRUE;
  END IF;
  ...
  IF p_role = 'influencer' THEN
    RETURN CASE
      ...
      WHEN p_old = 'accepted' AND p_new IN ('in_progress','cancelled') THEN TRUE
...
BEGIN
  ...
  IF NEW.net_amount IS NULL
     OR NEW.net_amount <= 0
     OR (TG_OP = 'UPDATE' AND OLD.total_amount IS DISTINCT FROM NEW.total_amount) THEN
...
BEGIN
  ...
  NEW.merchant_confirm_deadline := NULL;
  RETURN NEW;
END;
$$;
...
BEGIN
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO public.audit_orders (order_id, old_status, new_status, changed_by, change_source, metadata)
    VALUES (
      NEW.id,
      OLD.status,
      NEW.status,
...
BEGIN
  ...
  VALUES
    (p_user, p_type, p_title, p_content, p_rel_type, p_rel_id, p_url)
    ...
    CASE NEW.status
      ...
      WHEN 'cancelled' THEN
        v_user := CASE WHEN OLD.status = 'payment_authorized'
                       THEN NEW.merchant_id ELSE NEW.influencer_id END;
      ...
      WHEN 'review_pending' THEN
        ...
        v_content := 'Le client demande des modifications.';
      WHEN 'completed' THEN
        v_user := NEW.influencer_id;
      ...
      WHEN 'disputed' THEN
        ...
        v_content := 'Un litige est en cours.';
      WHEN 'finished' THEN
        v_user := NEW.influencer_id;
...
BEGIN
  ...
  IF NEW.sender_id NOT IN (v_p1, v_p2)
     OR NEW.receiver_id NOT IN (v_p1, v_p2) THEN
...
-- ============================================================================
-- 5. RPC FONCTIONS (ORDERS, WALLET, DISPUTES)
-- ============================================================================
...
BEGIN
  ...
  SET
    ...
    updated_at = NOW(),
    submitted_at = CASE
      WHEN p_new_status = 'submitted' THEN NOW() ELSE submitted_at END,
    completed_at = CASE
      WHEN p_new_status = 'completed' THEN NOW() ELSE completed_at END,
    cancelled_at = CASE
      WHEN p_new_status = 'cancelled' THEN NOW() ELSE cancelled_at END,
    ...
    END,
    dispute_reason = CASE
      WHEN p_new_status = 'disputed' THEN p_reason ELSE dispute_reason END,
  ...
  IF (p_new_status = 'cancelled' OR p_new_status = 'disputed')
     AND v_order.status IN ('completed','finished') THEN

    PERFORM 1 FROM public.revenues
    ...
    IF FOUND THEN
      ...
      VALUES (
        'error',
        'CRITICAL: Revenue reversion failed - Money already withdrawn',
  ...
  -- Log spécifique : cancel après accepted par l'influenceur
  IF p_new_status = 'cancelled'
     AND v_order.status = 'accepted'
     AND v_role = 'influencer' THEN
    INSERT INTO public.system_logs (event_type, message, details)
    VALUES (
      'workflow',
...
BEGIN
  ...
  IF p_delivery_url !~ '^https?://[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}.*' THEN
...
BEGIN
  ...
  -- Auto-cancel commandes non acceptées
  FOR rec IN
    SELECT id
    FROM public.orders
  ...
  -- Auto-complete commandes non validées par le merchant
  FOR rec IN
    SELECT id
    FROM public.orders
...
BEGIN
  ...
  WHERE influencer_id = v_user AND status = 'available';
...
BEGIN
  ...
  LOOP
    ...
    -- On consomme uniquement des lignes entières
...
BEGIN
  ...
  INTO v_pend_rev
  FROM public.revenues
  WHERE influencer_id = v_user AND status = 'pending';
  ...
  INTO v_withdrawn
  FROM public.revenues
  WHERE influencer_id = v_user AND status = 'withdrawn';
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO public.revenues (influencer_id, order_id, amount, net_amount, commission, status, available_at)
    VALUES (
      v_order.influencer_id,
      p_order_id,
      v_order.total_amount,
  ...
  PERFORM public.create_notification(
    CASE WHEN p_resolution = 'refund_merchant'
         THEN v_order.merchant_id ELSE v_order.influencer_id END,
    'dispute_resolved',
    'Litige résolu',
    CASE WHEN p_resolution = 'refund_merchant'
         THEN 'Résolu en votre faveur.' ELSE 'Fonds disponibles.' END,
    'order',
    p_order_id
  );
...
BEGIN
  ...
  INSERT INTO public.reviews (order_id, merchant_id, influencer_id, rating, comment)
  VALUES (p_order_id, v_user, v_order.influencer_id, p_rating, p_comment)
  RETURNING id INTO v_id;
...
BEGIN
  ...
  IF v_order.status NOT IN ('disputed','review_pending') THEN
    RAISE EXCEPTION 'Can only contest disputed or review_pending. Current: %',
      v_order.status;
  END IF;
...
BEGIN
  ...
  PERFORM public.create_notification(
    ...
    CASE WHEN p_status = 'accepted'
         ...
         ELSE 'Contestation refusée' END,
    CASE WHEN p_status = 'accepted'
         THEN 'Acceptée.' ELSE 'Refusée.' END,
    'contestation',
    p_contestation_id
  );
...
BEGIN
  IF auth.uid() IS NOT NULL THEN
    PERFORM public.apply_rate_limit('profile_view');
  END IF;
...
BEGIN
  IF auth.uid() IS NOT NULL THEN
    PERFORM public.apply_rate_limit('profile_share');
  END IF;
...
BEGIN
  ...
  SET is_read = TRUE,
      read_at = NOW(),
      updated_at = NOW()
  WHERE user_id = v_user AND is_read = FALSE;
...
BEGIN
  ...
  VALUES (
    ...
    CASE WHEN v_phone IS NOT NULL AND v_phone <> ''
...
BEGIN
  IF OLD.email IS DISTINCT FROM NEW.email
    ...
    UPDATE public.profiles
    SET
      email_encrypted = pgp_sym_encrypt(NEW.email, v_key),
...
-- Orders: ordre déterministe
DROP TRIGGER IF EXISTS trg_orders_01_snapshot ON public.orders;
DROP TRIGGER IF EXISTS trg_orders_02_enforce ON public.orders;
DROP TRIGGER IF EXISTS trg_orders_03_calculate ON public.orders;
DROP TRIGGER IF EXISTS trg_orders_04_stripe_sync ON public.orders;
DROP TRIGGER IF EXISTS trg_orders_05_audit ON public.orders;
DROP TRIGGER IF EXISTS trg_orders_06_notify ON public.orders;
DROP TRIGGER IF EXISTS trg_orders_07_auto_notify ON public.orders;

CREATE TRIGGER trg_orders_01_snapshot
BEFORE INSERT ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.snapshot_offer_on_order_create();

CREATE TRIGGER trg_orders_02_enforce
BEFORE INSERT ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.enforce_order_creation_rules();

CREATE TRIGGER trg_orders_03_calculate
BEFORE INSERT OR UPDATE OF total_amount, commission_rate ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.calculate_order_amounts();

CREATE TRIGGER trg_orders_04_stripe_sync
BEFORE UPDATE OF stripe_payment_status ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.sync_stripe_status_to_order();

CREATE TRIGGER trg_orders_05_audit
AFTER UPDATE OF status ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.audit_order_status_change();

CREATE TRIGGER trg_orders_06_notify
AFTER UPDATE OF status ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.notify_order_change();

CREATE TRIGGER trg_orders_07_auto_notify
AFTER UPDATE OF status ON public.orders
...
DROP TRIGGER IF EXISTS trg_messages_check ON public.messages;
...
CREATE TRIGGER trg_messages_check
BEFORE INSERT ON public.messages
...
-- Conversations
DROP TRIGGER IF EXISTS trg_conversations_canonical ON public.conversations;

CREATE TRIGGER trg_conversations_canonical
...
-- Profiles protection & updated_at
...
CREATE TRIGGER update_offers_modtime BEFORE UPDATE ON public.offers
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_orders_modtime BEFORE UPDATE ON public.orders
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_revenues_modtime BEFORE UPDATE ON public.revenues
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_withdrawals_modtime BEFORE UPDATE ON public.withdrawals
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_social_links_modtime BEFORE UPDATE ON public.social_links
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_reviews_modtime BEFORE UPDATE ON public.reviews
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_bank_accounts_modtime BEFORE UPDATE ON public.bank_accounts
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_portfolio_items_modtime BEFORE UPDATE ON public.portfolio_items
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_contestations_modtime BEFORE UPDATE ON public.contestations
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_notifications_modtime BEFORE UPDATE ON public.notifications
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Auth triggers (à créer côté schema auth)
...
SELECT
  u.id AS user_id,
  COALESCE(SUM(CASE WHEN o.status IN ('completed','finished') AND o.influencer_id = u.id THEN o.net_amount ELSE 0 END),0) AS total_earned,
  COALESCE(COUNT(DISTINCT CASE WHEN o.status IN ('completed','finished') AND o.influencer_id = u.id THEN o.id END),0) AS total_orders,
  COALESCE(u.completed_orders_count,0) AS completed_orders_count,
  COALESCE(u.average_rating,0) AS average_rating,
  COALESCE(u.total_reviews,0) AS total_reviews
FROM public.profiles u
LEFT JOIN public.orders o ON o.influencer_id = u.id
GROUP BY u.id;
...
-- PROFILES : accès complet seulement pour soi / admin (vue publique pour le reste)
...
ON public.conversations
FOR SELECT
USING (auth.uid() IN (participant_1_id, participant_2_id) OR public.is_admin());
...
ON public.orders
FOR SELECT
USING (auth.uid() IN (merchant_id, influencer_id) OR public.is_admin());
...
ON public.reviews
FOR SELECT
USING (is_visible = TRUE OR merchant_id = auth.uid() OR influencer_id = auth.uid() OR public.is_admin());
...
FOR SELECT
USING (auth.uid() IN (influencer_id, merchant_id) OR public.is_admin());
...
CREATE POLICY "system_logs_admin"
ON public.system_logs
FOR SELECT
USING (public.is_admin());

-- ============================================================================
-- 9. DONNÉES INITIALES (CATEGORIES)
-- ============================================================================

-- Catégories "thématiques"
INSERT INTO public.categories (name, slug, icon_name, description, sort_order) VALUES
('Mode & Fashion', 'mode-fashion', 'shirt', 'Vêtements, accessoires, tendances', 10),
('Beauté & Cosmétique', 'beaute-cosmetique', 'sparkles', 'Maquillage, soins, parfums', 20),
('Tech & Gadgets', 'tech-gadgets', 'smartphone', 'High-tech, accessoires, innovations', 30),
('Maison & Déco', 'maison-deco', 'home', 'Décoration, ameublement, jardin', 40),
('Sport & Fitness', 'sport-fitness', 'dumbbell', 'Équipement, nutrition, coaching', 50),
('Voyage & Lifestyle', 'voyage-lifestyle', 'plane', 'Hôtels, expériences, valises', 60),
('Enfants & Famille', 'enfants-famille', 'baby', 'Jouets, puériculture, vêtements', 70),
('Animaux', 'animaux', 'paw', 'Accessoires, nourriture, soins', 80),
('Alimentation & Boissons', 'alimentation', 'utensils', 'Produits gourmands, bio, boissons', 90),
('Art & Création', 'art-creation', 'palette', 'DIY, fournitures, œuvres', 100)
...
-- Catégories "formats de contenu"
INSERT INTO public.categories (name, slug, icon_name, description, sort_order) VALUES
  ('Story Instagram', 'story-instagram', 'instagram', 'Story temporaire 24h', 110),
  ('Post Instagram', 'post-instagram', 'image', 'Publication permanente feed', 120),
  ('Reel Instagram', 'reel-instagram', 'video', 'Vidéo courte verticale', 130),
  ('Vidéo TikTok', 'video-tiktok', 'tiktok', 'Vidéo courte TikTok', 140),
  ('Story Snapchat', 'story-snapchat', 'snapchat', 'Story Snapchat', 150),
  ('UGC Vidéo', 'ugc-video', 'camera', 'Contenu vidéo généré', 160),
  ('UGC Photo', 'ugc-photo', 'camera', 'Contenu photo généré', 170),
  ('Post LinkedIn', 'post-linkedin', 'linkedin', 'Publication LinkedIn', 180),
  ('Thread Twitter/X', 'thread-twitter', 'twitter', 'Thread X/Twitter', 190),
  ('Vidéo YouTube', 'video-youtube', 'youtube', 'Vidéo YouTube', 200),
  ('Short YouTube', 'short-youtube', 'youtube', 'Short YouTube', 210),
  ('Post Facebook', 'post-facebook', 'facebook', 'Publication Facebook', 220)
ON CONFLICT (slug) DO UPDATE
SET name = EXCLUDED.name,
    description = EXCLUDED.description,
    sort_order = EXCLUDED.sort_order,
...
-- ============================================================================
-- FIN SCRIPT V20.0 - MARKETPLACE INFLUENCEURS
-- ============================================================================
