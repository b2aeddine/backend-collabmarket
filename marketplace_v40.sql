...
-- ============================================================================
-- MARKETPLACE V40.0 - FULL SCHEMA (FIVERR-LIKE + AFFILIATION RULES)
-- ============================================================================
-- Base: V36 architecture with critical fixes
-- FIXES IN V40:
-- - reverse_commissions: properly handles partial refunds with pro-rata
-- - profiles RLS: added SELECT grant for authenticated users
...
-- ============================================================================
-- V40 FIX: reverse_commissions with proper partial refund support
-- ============================================================================
-- This function now correctly handles partial refunds by:
-- 1. Calculating pro-rata amounts based on refund ratio
-- 2. Tracking cumulative refunds to prevent exceeding order total
-- 3. Updating revenue records proportionally instead of cancelling entirely
-- 4. Setting correct refund_status ('partial' vs 'full')
-- ============================================================================
...
BEGIN
  ...
  -- Get current revenue records (only unlocked ones can be modified)
...
-- Views
-- public_profiles: Safe view for public profile data (no sensitive columns)
-- Use this view for marketplace listings, not the profiles table directly
...
-- Profiles
-- Users can only read their own complete profile (with sensitive data)
-- For public profile info, use public.public_profiles view
...
-- Views (safe public access)
-- anon and authenticated can read public profile info via the view
-- but NOT the full profiles table (which contains sensitive data)
...
-- V40 FIX: Added SELECT on profiles so users can read their own complete profile
-- RLS policy "Users see own private profile" restricts this to auth.uid() = id
